...
public class BookSetTest {
    ...
    public void theLargestSetOfBooksInOneBookSetIsOne() {
        BookSet bookSet = new BookSet(Lists.newArrayList(new HarryPotterBookType(BookType.One, 1)));
        assertEquals(BookSetType.One, bookSet.getBookSetType());
    }

    @Test
    public void theLargestSetOfBooksInBookSetWithTwoBooksTheSameIsOne() {
        BookSet bookSet = new BookSet(Lists.newArrayList(new HarryPotterBookType(BookType.One, 2)));
        assertEquals(BookSetType.One, bookSet.getBookSetType());
    }
    ...
    public void theLargestSetOfBooksInBookSetWithTwoBooksDifferentIsTwo() {
        BookSet bookSet = new BookSet(new BooksBuilder()
                .withBook(new HarryPotterBookType(BookType.One, 1))
    ...
    public void theLargestSetOfBooksInBookSetWithTwoBooksDifferentAndSomeBooksTheSameIsStillTwo() {
        BookSet bookSet = new BookSet(new BooksBuilder()
                .withBook(new HarryPotterBookType(BookType.One, 2))
                .withBook(new HarryPotterBookType(BookType.Two, 3))
                .build());
        assertEquals(BookSetType.Two, bookSet.getBookSetType());
    }
    ...
    public void shouldReturnEmptyBookSetWhenRemovingCurrentLargestBookSetFromSetWithOneBook() {
        // Given
        BookSet startingBookSet = new BookSet(Lists.newArrayList(new HarryPotterBookType(BookType.One, 2)));
        // When
        BookSet bookSet = startingBookSet.removeLargestSet();

        // Then
        BookSet expectedBookSet = new BookSet(Lists.newArrayList(new HarryPotterBookType(BookType.One, 1)));
    ...
    public void shouldCalculatePriceOfLargestSetOfBookSetWithFiveDifferentBooksAndAssortedOtherBooks() {
        BookSet bookSet = new BookSet(new BooksBuilder().withBook(new HarryPotterBookType(BookType.One, 99))
                .withBook(new HarryPotterBookType(BookType.Two, 400))
                .withBook(new HarryPotterBookType(BookType.Three, 542352))
                .withBook(new HarryPotterBookType(BookType.Four, 3453234))
                .withBook(new HarryPotterBookType(BookType.Five, 23))
                .build());
